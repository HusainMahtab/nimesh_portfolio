...
function Header() {
  ...
  // handle download resume function
  const handleDownloadResume = async () => {
    try {
      // Replace with your Google Drive direct download link
      const fileId = "15eVu6UjWa6w5sr75S56jpLgjOdbqtOoj";
      const resumeUrl = `https://drive.google.com/uc?export=download&id=${fileId}`;

      // Open in new tab as Google Drive might redirect
      window.open(resumeUrl, "_blank");

      toast.success("Opening resume download...");
    } catch (error) {
  ...
  return (
    <header className="bg-white text-black border-b-2 border-b-[#FF0000] p-4 px-6 md:px-2">
      {/* Desktop View */}
      <div className="hidden md:flex justify-between items-center text-lg font-serif font-bold">
        ...
        <nav className="flex gap-6">
          <Link
            className="hover:border-b hover:border-b-[#FF0000] duration-500 mx-3"
            to="/"
          >
            Home
          </Link>
          ...
          <Link
            className="hover:border-b hover:border-b-[#FF0000] duration-500"
            to="/contact"
          >
            Contact
          </Link>
        ...
        <div className="flex gap-4 items-center">
          <div className="grid place-content-center place-items-center">
            <FaArrowCircleDown
              ...
              className="text-[#FF0000] hover:text-red-700 text-2xl cursor-pointer"
          ...
          {user?._id ? (
            <div>
              <button
                className="border-2 border-[#FF0000] p-2 font-bold bg-[#FF0000] text-white rounded"
          ...
          <button
            className="p-2 border-2 border-[#FF0000] text-[#FF0000] font-bold hover:bg-[#FF0000] hover:text-white"
          ...
          {user?.profilePic ? (
            <div className="grid place-content-center place-items-center">
              ...
              {user.role === "ADMIN" && (
                <p
                  className="text-[#FF0000] text-sm cursor-pointer hover:underline"
      ...
      <div className="flex md:hidden gap-2 justify-between items-center">
        ...
        <div className="flex gap-2 justify-center items-center">
          ...
          <button
            className="p-1 border-2 border-[#FF0000] text-[#FF0000] font-bold hover:bg-[#FF0000] hover:text-white"
            onClick={() => navigate("/signup")}
          >
            Signup
          </button>
